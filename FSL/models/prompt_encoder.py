...
# from encoder_layer import PromptEncodeLayer
...
class PromptEncodeLayer(nn.Module):
    def __init__(self, d_k=32, d_v=32, d_model=256, d_ff=1024, n_heads=3, dropout=0.5):
        super(PromptEncodeLayer, self).__init__()
...
class PromptEncoder(torch.nn.Module):
    def __init__(self, spell_length, hidden_size):
        ...
        # LSTM
        # self.encode_layers = torch.nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size // 2, num_layers=self.n_layers,
        #                                dropout=0.5, bidirectional=True, batch_first=True)

        # Transformer
        self.encode_layers = nn.ModuleList([PromptEncodeLayer(d_model=hidden_size) for _ in range(self.n_layers)])

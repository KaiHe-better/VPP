...
class Mtb(FewShotREModel):
    ...
    def __init__(self, sentence_encoder, use_dropout=True, combiner="max"):
        ...
        if type(sentence_encoder).__name__=='RobertaSentenceEncoder':
            my_encoder = sentence_encoder.roberta
        elif type(sentence_encoder).__name__=='BERTSentenceEncoder':
            my_encoder = sentence_encoder.bert
        else:
            raise NotImplementedError
        self.layer_norm = torch.nn.LayerNorm(my_encoder.config.hidden_size * (2 if sentence_encoder.cat_entity_rep else 1))
